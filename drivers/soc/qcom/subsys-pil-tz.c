...
/* vivo marongqiang add for subsystem crash collect start */
#define MODULE_ID		"K575"
#define EVENT_ID		"00059|012"
#define MAX_SUBSYS_NAME_LEN 64
#define MAX_DATA_REASON_LEN 300
static struct work_struct subsys_crash_work;
static char subtype[MAX_SUBSYS_NAME_LEN];
static char crash_reason[MAX_SSR_REASON_LEN];
#if !defined(VIVO_PROJECT_MODEL)
extern int track_event(char *modelId, char *filename, char *data);
#endif
/* vivo marongqiang add for subsystem crash collect end */
...
static void subsys_crash_collect(struct work_struct *work)
{
	char ex_data[MAX_DATA_REASON_LEN];
	char *pos = ex_data;
	//int len = 0;
	int i = 0;
	char subtype_name[][10] = {"modem", "adsp", "cdsp", "slpi", "others"};
	int subtype_id = sizeof(subtype_name)/sizeof((subtype_name[0]));
	/* 1:modem, 2:adsp, 3:cdsp, 4:slpi, 5:other subsystem*/
	/* The message format as follow: */
	/* logsystem [{"moduleid":"3300","eventId":"00059|012","extype":"1","subtype":"1_2_3_4_5","times":"1","exInfo": "name:reason"}] */
	/* vcode moduleid&eventId&extype:1&subtype:1_2_3_4_5&exInfo:name:reason */

	memset(ex_data, '\0', MAX_DATA_REASON_LEN);
	//len = scnprintf(pos, 100, "[{\"moduleid\":\"%s\",\"eventId\":\"%s\",\"extype\":\"1\",\"subtype\":\"", MODULE_ID, EVENT_ID);
	//pos += len;
	for (i = 0; i < subtype_id; i++) {
		if (strncmp(subtype_name[i], subtype, strlen(subtype_name[i])) == 0) {
			subtype_id = i + 1;
			break;
		}
	}
	scnprintf(pos, 200, "extype:1&subtype:%d&exInfo:%s:%s", subtype_id, subtype, crash_reason);
	pr_err("%s subsystem failure ex_data: %s.\n", __func__, ex_data);
	return;
}
/* vivo marongqiang add for subsystem crash collect end */
...
static void log_failure_reason(const struct pil_tz_data *d)
{
	...
	/* vivo marongqiang add for subsystem crash collect start */
	memset(crash_reason, '\0', MAX_SSR_REASON_LEN);

	if (d->smem_id == -1) {
		strlcpy(crash_reason, "pil_tz_data smem_id equals '-1'", (MAX_SSR_REASON_LEN - 1));
		schedule_work(&subsys_crash_work);
		return;
	}
	...
	if (!smem_reason[0]) {
		...
		strlcpy(crash_reason, "SFR: (unknown, empty string found)", (MAX_SSR_REASON_LEN - 1));
		schedule_work(&subsys_crash_work);
		return;
	}

	strlcpy(reason, smem_reason, MAX_SSR_REASON_LEN);
	pr_err("%s subsystem failure reason: %s.\n", name, reason);
	memset(subtype, '\0', MAX_SUBSYS_NAME_LEN);
	strlcpy(subtype, name, MAX_SUBSYS_NAME_LEN);
	strlcpy(crash_reason, smem_reason, (MAX_SSR_REASON_LEN - 1));
	schedule_work(&subsys_crash_work);
	/* vivo marongqiang add for subsystem crash collect end */
}
...
{
	/* vivo marongqiang add for subsystem crash collect start */
	pr_err("%s subsystem failure collect.\n", __func__);
	INIT_WORK(&subsys_crash_work, subsys_crash_collect);
	/* vivo marongqiang add for subsystem crash collect end */
