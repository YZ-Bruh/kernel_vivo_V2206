		...
		isolate_mode_t mode, enum lru_list lru)
{
	...
	*nr_scanned = total_scan;
	trace_mm_vmscan_lru_isolate(sc->reclaim_idx, sc->order, nr_to_scan,
					...
					sc->special ? sc->special : lru);
	update_lru_sizes(lruvec, lru, nr_zone_taken);
#ifdef CONFIG_FCACHE_CTRL
	if (sc->special)
		update_lru_sizes(lruvec, sc->special, nr_zone_taken);
#endif
	return nr_taken;
}
...
#ifdef CONFIG_FCACHE_CTRL
		if (lru == LRU_ACTIVE_FILE && sc->special) {
			update_lru_size(lruvec, sc->special, page_zonenum(page), nr_pages);
			list_move(&page->lru, &lruvec->lists[sc->special]);
		} else {
			       ...
			       enum lru_list lru)
{
...
#ifdef CONFIG_FCACHE_CTRL
		if (lru == LRU_ACTIVE_FILE && !sc->special && fctrl_en) {
			if (trylock_page(page)) {
				struct address_space *mapping = NULL;
				struct inode *inode = NULL;

				if ((mapping = page_mapping_file(page)) != NULL) {
					inode = mapping->host;

					if (is_vital_inode(inode)) {
						list_add(&page->lru, &l_vital);
						trace_mm_vmscan_skip_page(sc->priority, inode,
								page, page_lru(page),
								0, nr_taken);
						SetPageVital(page);
						unlock_page(page);
...
#ifdef CONFIG_FCACHE_CTRL
	nr_activate = move_active_pages_to_lru(lruvec, &l_active, &l_hold, sc, lru);

	if (lru == LRU_ACTIVE_FILE && !sc->special) {
		sc->special = LRU_VITAL;
		nr_activate += move_active_pages_to_lru(lruvec, &l_vital, &l_hold,
				sc, lru);
#ifdef CONFIG_FCACHE_CTRL_DATA
		sc->special = LRU_STAPLE;
		nr_activate += move_active_pages_to_lru(lruvec, &l_staple, &l_hold,
				sc, lru);
#endif
		sc->special = 0;
	}

	nr_deactivate = move_active_pages_to_lru(lruvec, &l_inactive, &l_hold, sc, lru - LRU_ACTIVE);
#else
	...
	trace_mm_vmscan_lru_shrink_active(pgdat->node_id, nr_taken, nr_activate,
			nr_deactivate, nr_rotated, sc->priority, file, sc->special);
}
...
#ifdef CONFIG_FCACHE_CTRL
			if (is_file_lru(lru) && current_is_kswapd() &&
					sc->priority <= mm_kswap && fctrl_en) {
				unsigned long inactive, active;
				unsigned long staple = 0, vital = 0;
				bool is_low;

				inactive = lruvec_lru_size(lruvec, LRU_INACTIVE_FILE, MAX_NR_ZONES);
				active = lruvec_lru_size(lruvec, LRU_ACTIVE_FILE, MAX_NR_ZONES);
#ifdef CONFIG_FCACHE_CTRL_DATA
				staple = lruvec_lru_size(lruvec, LRU_STAPLE, MAX_NR_ZONES);
#endif
				vital = lruvec_lru_size(lruvec, LRU_VITAL, MAX_NR_ZONES);

				is_low = lru_file_is_low(inactive + active - staple - vital);
				if (is_low)
					nr_low++;

#ifdef CONFIG_FCACHE_CTRL_DATA
				if (is_low || staple_list_is_full(staple)) {
					sc->special = LRU_STAPLE;
					shrink_active_list(SWAP_CLUSTER_MAX >> 1, lruvec, sc, lru);
					sc->special = 0;
				}
#endif

				if (is_low || vital_list_is_full(vital)) {
					sc->special = LRU_VITAL;
					shrink_active_list(SWAP_CLUSTER_MAX >> 1, lruvec, sc, lru);
					sc->special = 0;
				}
...
retry:
	...
	if (global_reclaim(sc)) {
		...
		if (likely(current->signal->oom_score_adj == 0))
			__count_mbd_event(ALLOCSTALL_TOP);
		if (unlikely(sc->order > 3))
			__count_mbd_event(ALLOCSTALL_GO3);
		else if (unlikely(sc->order == 3))
			__count_mbd_event(ALLOCSTALL_EO3);
	}
	t3 = local_clock();
#else
	if (global_reclaim(sc))
...
#ifdef CONFIG_MEM_BIGDATA
	__count_mbd_event(KSWAPD_WAKEUP_COUNT);
#endif
