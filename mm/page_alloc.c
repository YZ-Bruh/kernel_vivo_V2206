...
extern unsigned int system_boot_time;
extern unsigned int bsp_test_mode;
#endif

#ifdef CONFIG_MEM_BIGDATA
#include <linux/mem_big_data.h>
#ifndef CONFIG_BIG_ORDER_MONITOR
#define SYSTEM_BOOT_TIME 	200	/*  200s */
					...
					struct per_cpu_pages *pcp)
{
	...
	list_for_each_entry_safe(page, tmp, &head, lru) {
		...
		if (unlikely(isolated_pageblocks) || is_highatomic_pool(mt))
...
#endif
#ifdef CONFIG_HIGHATOMIC_POOL
	[MIGRATE_HIGHATOMIC_POOL] = { MIGRATE_TYPES }, /* Never used */
#endif
#ifdef CONFIG_MEMORY_ISOLATION
				...
				unsigned int alloc_order)
{
	...
	if (!is_migrate_highatomic(mt) && !is_migrate_isolate(mt)
	    && !is_migrate_cma(mt) && !is_highatomic_pool(mt)) {
	...
	for (i = 0; i < count; ++i) {
		...
		if (is_migrate_cma(get_pcppage_migratetype(page)))
			__mod_zone_page_state(zone, NR_FREE_CMA_PAGES,
					      -(1 << order));
		else if (is_highatomic_pool(get_pcppage_migratetype(page)))
			__mod_zone_page_state_internel(zone, NR_FREE_HIGHATOMIC_POOL_PAGES,
						  -(1 << order));
	}
...
#ifdef CONFIG_HIGHATOMIC_POOL
			page = __rmqueue_smallest(zone, order, MIGRATE_HIGHATOMIC_POOL);
			if (page)
				trace_mm_page_alloc_zone_locked(page, order, MIGRATE_HIGHATOMIC_POOL);
			else {
...
#ifdef CONFIG_HIGHATOMIC_POOL
	if (highatomic_pool_enabled(z) && !alloc_harder)
		free_pages -= zone_page_state(z, NR_FREE_HIGHATOMIC_POOL_PAGES);
#endif
...
#ifdef CONFIG_HIGHATOMIC_POOL
		if (highatomic_pool_enabled(z) && alloc_harder &&
			!list_empty(&area->free_list[MIGRATE_HIGHATOMIC_POOL]))
			return true;
#endif
	}
	return false;
...
void warn_alloc(gfp_t gfp_mask, nodemask_t *nodemask, const char *fmt, ...)
{
	...
	if (fatal_signal_pending(current) && !(gfp_mask & __GFP_NOFAIL))
		__count_mbd_event(ALLOC_FAILD_KILL);

	__count_mbd_event(ALLOC_FAILD);
#endif
	va_start(args, fmt);
	vaf.fmt = fmt;
	vaf.va = &args;
...
#ifdef CONFIG_MEM_BIGDATA
	if (unlikely(order == 3)) {
		__count_mbd_event(ALLOC_EO3);
	} else if (order > PAGE_ALLOC_COSTLY_ORDER &&
		((jiffies-INITIAL_JIFFIES)/CONFIG_HZ) > system_boot_time) {

		if ((gfp_mask & __GFP_NORETRY) && (gfp_mask & __GFP_NOWARN))
			goto mbd_next;
		__count_mbd_event(ALLOC_GO3);
	}
mbd_next:
#endif

#ifdef CONFIG_BIG_ORDER_MONITOR
	if (unlikely(bsp_test_mode) && order > PAGE_ALLOC_COSTLY_ORDER &&
		((jiffies-INITIAL_JIFFIES)/CONFIG_HZ) > system_boot_time) {

		if (unlikely(gfp_mask & __GFP_NOMONITOR))
			goto next;
		...
		pr_err("order:%d > %d may alloc failed, time:%lus; please fix this issue\n",
			order, PAGE_ALLOC_COSTLY_ORDER, (jiffies-INITIAL_JIFFIES)/CONFIG_HZ);
		dump_stack();

		mem_big_order_monitor(gfp_mask, order);
	}
next:
#endif
...
out:
	...
	if (unlikely(order >= 3)) {
		mbd_t1 = local_clock();
		mbd_time_geo3_event(mbd_t1 - mbd_t0);
	}
#endif
...
#ifdef CONFIG_HIGHATOMIC_POOL
	, global_zone_page_state(NR_FREE_HIGHATOMIC_POOL_PAGES)
#endif
		);
...
#ifdef CONFIG_HIGHATOMIC_POOL
			, K(zone_page_state(zone, NR_FREE_HIGHATOMIC_POOL_PAGES))
#endif
			);
...
static void __setup_per_zone_wmarks(void)
{
	...
	for_each_zone(zone) {
		...
		if (highatomic_pool_enabled(zone))
			init_highatomic_pool(zone, MIGRATE_HIGHATOMIC_POOL);
#endif
		spin_unlock_irqrestore(&zone->lock, flags);
	}
